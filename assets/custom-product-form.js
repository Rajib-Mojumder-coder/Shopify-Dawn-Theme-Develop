 ...
 *
 * - Detecting variant change
 * - Reading variant price data
 * - Updating already-rendered HTML
 * - Sale state
 * - Savings calculation
 *
 * Liquid remains responsible for:
 *
 * - Money format
 * - Currency format
 * - Initial rendering
...
function updateProductPrice(product) {
    ...
    /*---------------------------------------------------------*
     * 5. Verify product scope
     *
     * Prevent another product on the same page from
     * updating this price component.
     *---------------------------------------------------------*/
    ...
    if (
        currentProductId &&
        scopeProductId &&
        currentProductId !== scopeProductId
    ) {
        return;
    }
     ...
     * 8. Round price
     *
     * Liquid controls whether this option is enabled.
     *---------------------------------------------------------*/
     ...
     * IMPORTANT:
     *
     * These formats are generated by Liquid and stored
     * in data attributes.
     *---------------------------------------------------------*/
    ...
    const moneyWithCurrencyFormat =
        priceScope.dataset.moneyWithCurrencyFormat || "";


    /*---------------------------------------------------------*
     * 13. Select correct Shopify format
     *---------------------------------------------------------*/

    if (
        priceFormat === "money_with_currency" &&
    ...
    function formatMoney(amount) {
        ...
        if (
            ...
            /*------------------------------------------------*
             * Remove trailing zeros when requested
             *------------------------------------------------*/
         ...
         * Basic fallback.
         *
         * This should rarely be used because the
         * Liquid money format is already supplied.
         */
    ...
    if (currentPriceElement) {
         ...
         *
         * This preserves Shopify-generated money markup
    ...
    if (savingsAmountElement) {
        ...
        if (onSale) {
            ...
            savingsAmountElement.innerHTML =
                "Save " +
